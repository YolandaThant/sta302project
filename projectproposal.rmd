...
\section*{Exploratory Data Analysis (EDA) - Data Description}
...
When determining the market price of diamonds, jewelers justify their valuation with reference to the physical attributes of the diamond such as its cut, carat, colour, and size. Thus, price is an appropriate selection for the response variable as it is dependent on the chosen predictors. Further, price is a continuous numerical value measured in US Dollars, making it suitable for linear regression analysis.  

```{r}
## Numerical Summary of Response Variable
response_numeric_sum <- data.frame(
  Statistic = c("Min.", "1st Qu.", "Median", "Mode", "Mean", "Standard Deviation", "3rd Qu.", "Max."),
...
```{r}
## Numerical Summary of Predictor Variables
## Table & Transpose
...
numeric_sum$Statistic <- c("Min.", "1st Qu.", "Median", "Mean", "3rd Qu.", "Max.")
...
kable(numeric_sum, caption = "Numerical Summary of Predictor Variables", row.names=FALSE)
...
(carat_hist | table_hist | x_hist) / (price_color_box | price_clarity_box)
sd(diamonds_cleaned$price)
Mode(diamonds_cleaned$price)
```
...
(resid_fitted | price_fitted | price_carat) / 
  (price_color | price_color_box) / 
  (price_clarity | price_clarity_box) / 
  (price_table | price_x) / 
