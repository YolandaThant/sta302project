...
plot(mod.hii, type="h", ylab="hii", main="Levarage Points")
...
# Identify points where |standardized residual| > 2
## length(which(abs(transformed_sresid) > 2))
## 2229 entries
...
outliers <- which(abs(transformed_sresid) > 2)
...
## length(intersect(high_lev_pts, influential))    ## 794 pts
## length(intersect(outliers, influential))        ## 1686 pts
```
...
Model Validation: 80% Training Set & 20% Testing Set
...
mse_model_4 <- compute_test_mse(
  ~ log(carat) + color + clarity + length + width_transformed,
  data = diamonds_cleaned,
  lambda = 0.1414141,
...
kable(validation_results, caption = "Model Validation: Test Set MSE Comparison")
```
